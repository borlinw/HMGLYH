...
function addJhjxxh(){
	...
	$("#jhjxTable").find(".data-row").remove();	
	// 获取价格列表
	getGljdjs(ids,bmcode,function(data,html){
	...
	if( data && data.length > 0 ){
		...
		$.each(data,function(i,d){
			...
			html += "<tr class='datagrid-row data-row'>" +
						...
						"</td>" +
						"<td>" +
							"<div style='height:auto;text-align:center' class='datagrid-cell'>"+d.gg+"</div>" +
						"</td>" +
						"<td>" +
							"<div style='height:auto;text-align:center' class='datagrid-cell'>" +
...
function reCalculate1(){
	...
	$("#jhgr").val((pgsl*grde).toFixed(doubleFixed)); // 计算计划工日
...
function reCalculate2(){
	...
	$.each(trs,function(i,d){
		...
		if( wcgcl != "" && (!isNaN(wcgcl))) {
			...
			$(d).find(".rygrCls").val(parseFloat(rygr).toFixed(doubleFixed));
		}
	});
	
	$("#zysl").val(parseFloat(zyslTotal).toFixed(doubleFixed));
	$("#zgr").val(parseFloat(grTotal).toFixed(doubleFixed));
	...
	for( i ; i < trEs.length ; i++ ) {
		...
		$(trEs[i]).find(".gljsl").val(parseFloat(sl).toFixed(doubleFixed));
		clfTotal += parseFloat(dj) * parseFloat(sl);
	}
	$("#clfTotal").val(parseFloat(clfTotal).toFixed(moneyFixed));
	// 计算计划消耗机械费
...
function reCalculateRygcl(){
	...
	if(trs.length > 1 ) {
		...
		var wcl = ( zysl / len );
		$.each(trs,function(i,d){
			$(d).find(".rygclCls").val( wcl.toFixed(doubleFixed));
			$(d).find(".rygrCls").val( (wcl*grde).toFixed(doubleFixed));
		});
	}else{
		$("#zysl").val(0.000);
		$('#rgf').val(0.000);
...
function rygrBlur(){
	...
	if(trs.length > 1 ) {
		$.each(trs,function(i,d){
			var value =  $(d).find(".rygrCls").val();
			if((!isNaN(value) || value > 0 )) {
				zgr += parseFloat(value);
			}
		});
	}
	$("#zgr").val(parseFloat(zgr).toFixed(doubleFixed));
	$("#rgf").val(parseFloat(zgr*rgdj).toFixed(moneyFixed));
}
